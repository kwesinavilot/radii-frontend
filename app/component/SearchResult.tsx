...
//             <Markdown remarkPlugins={[remarkGfm]}>{result.insight}</Markdown>
//           </div>
//         )}
//       </div>
//     );
...
const SearchResult: React.FC<SearchResultProps> = ({ result, onSaveChart }) => {
  const [viewType, setViewType] = useState("Text");
  const [showInsights, setShowInsights] = useState(true);
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex space-x-2">
          <button
            ...
            onClick={() => {
              if (result.chart_data) {
                onSaveChart(result.chart_data, viewType, searchID);
              }
            }}
          >
            <IoSaveOutline className="mr-1" />
